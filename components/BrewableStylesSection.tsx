  ...
  if (style.needsAcid) {
    ...
    if (hco3) {
      items.push({
        ...
        detail: `${Math.round(hco3.current)} → ${Math.round(hco3.rangeMax)} mg/L  (−${Math.round(hco3.current - hco3.rangeMax)})`,
  ...
  if (style.needsBicarbonate) {
    ...
    if (hco3) {
      items.push({
        ...
        detail: `${Math.round(hco3.current)} → ${Math.round(hco3.rangeMin)} mg/L  (+${Math.round(hco3.rangeMin - hco3.current)})`,
  ...
  if (style.needsSalts) {
    ...
    if (so4 && so4.current < so4.rangeMin) {
      items.push({
        ...
        detail: `${Math.round(so4.current)} → ${Math.round(so4.target)} mg/L  (+${Math.round(so4.rangeMin - so4.current)})`,
    ...
    if (cl && cl.current < cl.rangeMin) {
      items.push({
        ...
        detail: `${Math.round(cl.current)} → ${Math.round(cl.target)} mg/L  (+${Math.round(cl.rangeMin - cl.current)})`,
    ...
    if (ca && ca.current < ca.rangeMin && !so4AlreadyLow && !clAlreadyLow) {
      items.push({
        ...
        detail: `${Math.round(ca.current)} → ${Math.round(ca.target)} mg/L  (+${Math.round(ca.rangeMin - ca.current)})`,
    ...
    <div className={`rounded-lg calculator:rounded-none border calculator:border-2 transition-colors ${
      ...
      >
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <span className="text-[11px] sm:text-xs font-mono font-bold text-gray-500 dark:text-gray-400 shrink-0">{style.bjcpCode}</span>
          <span className="flex-1 text-sm text-gray-900 dark:text-gray-100 calculator:text-calc-text font-mac leading-snug min-w-0 truncate">{style.name}</span>
          {specialiteNote && (
            <span className="text-xs text-gray-400 dark:text-gray-500 italic hidden lg:block mr-2 shrink-0">{specialiteNote}</span>
          )}
          {canExpand && (
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-1 ml-[calc(0.5rem+1rem)] sm:ml-[calc(0.75rem+1.25rem)]">
          ...
          {style.brewersFriendSlug && (
            <a
              ...
              className="inline-flex items-center gap-1 text-[11px] sm:text-xs px-1.5 sm:px-2 py-0.5 rounded border font-medium text-[#2563FF] dark:text-blue-400 border-[#2563FF]/30 dark:border-blue-400/30 bg-blue-50/50 dark:bg-blue-900/10 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
            >
              Recettes
              ...
              <div className={`rounded-lg calculator:rounded-none border px-2.5 py-2 ${adjustmentPlanBoxClass(style.status)}`}>
                ...
                <div className="space-y-2">
                  {adjustmentItems.map((item, i) => (
                    <div key={i} className={`flex items-start gap-2 text-xs ${i > 0 ? 'border-t border-gray-200/50 dark:border-gray-600/30 pt-2' : ''}`}>
                      <span className={`mt-0.5 shrink-0 ${
                        item.type === 'ok' ? 'text-emerald-500' :
                        item.type === 'dilute' ? 'text-red-400' :
                        item.type === 'reduce' ? 'text-amber-500' :
                        'text-blue-500'
                      }`}>
                        {item.type === 'ok' ? (
                          ...
                          <Icons.ExclamationTriangleIcon className="w-3.5 h-3.5" />
                        ) : (
                          <Icons.PlusCircleIcon className="w-3.5 h-3.5" />
                        )}
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-800 dark:text-gray-100 calculator:text-calc-text leading-snug">
                          {item.salt}
                        </p>
                        ...
                        {item.sensory && (
                          <p className="text-amber-700 dark:text-amber-400 calculator:text-calc-text italic mt-0.5 leading-snug">
                            Sinon : {item.sensory}
                          </p>
                        )}
                      </div>
    ...
    <div className="bg-white dark:bg-gray-800 calculator:bg-calc-bg-card rounded-xl calculator:rounded-none border border-gray-200 dark:border-gray-700 calculator:border-calc-border shadow-sm calculator:shadow-mac overflow-hidden">
      ...
      >
        <h4 className="font-bold text-base sm:text-lg text-gray-900 dark:text-gray-100 calculator:text-calc-text flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs sm:text-sm font-normal px-2 py-0.5 rounded-full calculator:rounded-none bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50">
              ...
              {([
                ...
                <label key={key} className="flex items-start gap-2.5 sm:gap-3 cursor-pointer group">
                  ...
                  <div className="min-w-0">
                    <span className="text-[13px] sm:text-sm font-medium text-gray-800 dark:text-gray-100 calculator:text-calc-text font-mac group-hover:text-[#2563FF] transition-colors">
                      {label}
                    </span>
                    <p className="text-[11px] sm:text-xs text-gray-500 dark:text-gray-400 font-mac mt-0.5">{sub}</p>
                  </div>
                </label>
              ))}
            </div>
            <p className="mt-3 text-[11px] sm:text-xs text-gray-400 dark:text-gray-500 italic font-mac">
