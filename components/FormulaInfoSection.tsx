  ...
  return (
    <div className="mt-6">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-900/30 calculator:bg-calc-bg-surface hover:bg-gray-100 dark:hover:bg-gray-700/50 calculator:hover:bg-calc-bg-card rounded-lg border border-gray-200 dark:border-gray-700 calculator:border-calc-border transition-colors"
        aria-expanded={open}
      >
        ...
        <Icons.ChevronRightIcon
          className={`w-4 h-4 text-gray-400 calculator:text-calc-text-muted transition-transform duration-200 ${open ? 'rotate-90' : ''}`}
        />
      </button>
          ...
          {entries.map((entry, idx) => (
            <div key={idx} className="px-4 py-4 space-y-2">
              ...
              {entry.formulas.length > 0 && (
                <div className="rounded-md bg-white dark:bg-gray-900 calculator:bg-calc-bg-surface border border-gray-200 dark:border-gray-700 calculator:border-calc-border px-3 py-2 space-y-0.5">
